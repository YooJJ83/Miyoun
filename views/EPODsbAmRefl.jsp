...
<script type="text/javascript">
	...
	$( function() {
		...
		$("#btn_save").click(function(){			
			...
			var dsbAm = "<input type='text' name='dsbAm' id='dsbAm' value=''/>";
            ...
            if(sendTgtIdx ==1){
            	...
            	$("#jqgrid").setRowData(sendTgtIdx, {dsbAm:'5,315,000'});            	
            	$("#jqgrid").setRowData(sendTgtIdx, {repoSubmYn:'Y'});
            }else{  
            	$("#jqgrid").setRowData(sendTgtIdx, {seq:'2'});
            	$("#jqgrid").setRowData(sendTgtIdx, {yosa:'2019'});
            	$("#jqgrid").setRowData(sendTgtIdx, {dpnm:'정보전산원'});
            	$("#jqgrid").setRowData(sendTgtIdx, {pnn:'유재준'});            	
            	$("#jqgrid").setRowData(sendTgtIdx, {dsbAm:dsbAm});            	
            	$("#jqgrid").setRowData(sendTgtIdx, {repoSubmYn:'Y'});
		...
		$("#jqgrid").jqGrid({
			datatype : "json", 
			mtype : "post", 
			colNames : [ '','순번','학년도','부서명','성명','보고서제출<BR>여부','지급액반영<BR>여부','지급액'],//60
			colModel : [
				{name : 'flag',	    index : 'flag',     align : 'center', editable : false,  hidden : true},
				{name : 'seq',	    index : 'seq',      align : 'center', editable : false,  width:  40},
				{name :	'yosa',     index : 'yosa',     align : 'center', editable : false,  width:  60   },
				{name : 'dpnm', 	index : 'dpnm', 	align : 'center', editable : false,  width:  200  },
				{name : 'pnn', 	  	index : 'pnn', 		align : 'center', editable : false,  width:  80  },				
				{name : 'repoSubmYn',index : 'repoSubmYn',align : 'center', editable : false,  width:  80  },
				{name : 'dsbAmYn',index : 'dsbAmYn',align : 'center', editable : false,  width:  80  },				
				{name : 'dsbAm',index : 'dsbAm',align : 'center', editable : false,  width:  100  }				
			],  
		...
		<div id="container">			
			<div class="ui-section">
				<div class="ui-section-header">
					<h2 class="title">지급액반영</h2>
					<ul class="action">
						<li>
							<span class="button"><input type="image" id="btn_exceluld2" name="btn_exceluld2" alt="엑셀업로드" title="엑셀업로드" src="/images/core/btn/btn-exceluld2.gif" /></span>
						</li>	
						<li>
